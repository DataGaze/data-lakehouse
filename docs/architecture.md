...
DataGaze data platform là một medallion lakehouse quy mô home-lab: dữ liệu thị trường chứng khoán Việt Nam (SSI) được thu thập bởi `ssi-connection` trên bizfly VPS, đẩy lên **Cloudflare R2** làm single source of truth, rồi được `data-pipeline` (Prefect 3 + Polars + dbt) kéo về LXC `lakehouse-gold` để chuyển hóa qua 3 tầng Bronze → Silver → Gold. Consumer chính là `MarketPulse` (phân tích + Telegram). Throughput hiện tại ~50MB/day (stock), mở rộng sang BĐS và multi-domain trong các quý tới.
...
| `data-pipeline`      | Runtime logic: ingestion, Polars, dbt, Prefect     | Processor  |
| `lakehouse-gold`     | LXC 202 Proxmox — Postgres + Prefect worker        | Storage    |
...
SSI ─► bizfly VPS (ssi-connection, cron 17:03) ─► Cloudflare R2 (SoT)
                                                        │
                                                        ▼
           lakehouse-gold (LXC 202) ◄── Prefect 3 flow 17:30 ──┘
                │
...
graph TB
    ...
    subgraph Homelab["Home Lab — Proxmox K3s"]
        subgraph LXC201["LXC 201"]
            Prefect[Prefect 3 Server<br/>API + UI + scheduler]
        end
...
3. **17:30 — Prefect flow trigger** (LXC 201). Prefect server phát flow run tới worker trên LXC 202.
4. **Bronze ingestion** (Polars, LXC 202). Worker đọc R2 qua boto3, ghi as-is vào `s3://lakehouse/bronze/stock/{YYYY-MM-DD}/{channel}/{exchange}.parquet` trên SeaweedFS. Không mutate — Bronze là immutable replay layer.
...
- R2 unreachable → Prefect flow fail ở step 4; bizfly vẫn buffer Parquet local, catch-up khi R2 phục hồi.
...
| Orchestration   | Prefect 3 (LXC 201 server, 202 worker) | Python-native, retries/schedules/alerts built-in, nhẹ hơn Airflow cho solo-dev                           | [ADR D4](./adr/2026-04-13-platform-decisions.md#d4-orchestration--transform-stack) |
...
| Recovery (replay từ R2)      | RPO = 1 day, RTO < 2h                           | R2 retain raw ≥ 90 days; Prefect flow có `--backfill` mode             |
| Alert latency                | < 5 phút khi flow fail                          | Prefect → Telegram bot                                                 |
...
|-------------|--------------|-------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------|
| **P0 Now**  | Đang chạy    | 1 domain (stock), 50MB/day, 1 consumer primary                                | Giữ nguyên: Postgres + SeaweedFS + Prefect single-worker                                          |
| **P1 Multi-domain** | Q2–Q3 2026 | Thêm BĐS (W14+ per README) — volume vẫn < 500MB/day                     | Thêm dbt source cho `bds`, Prefect flow riêng; Postgres vẫn đủ                                    |
...
- Ownership của Prefect flow khi thêm BĐS — share flow hay tách theo domain.
